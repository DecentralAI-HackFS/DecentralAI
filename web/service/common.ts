...
> = ({ url, params }) => {
  ...
  }) as Promise<UserProfileOriginResponse>;
};
...
export const fetchTenantInfo: Fetcher<TenantInfoResponse, { url: string }> = ({
  url,
}) => {
...
export const updateProviderAIKey: Fetcher<
  UpdateOpenAIKeyResponse,
  { url: string; body: { token: string | ProviderAzureToken } }
> = ({ url, body }) => {
...
export const requestLoginMessage: Fetcher<
  any,
  { url: string; body: { chainId: number; address: string } }
> = ({ url, body }) => {
  return post(url, { body }) as Promise<any>;
};

export const walletLogin: Fetcher<
  any,
  {
    url: string;
    body: { message: string; signature: string; remember_me: boolean };
  }
> = ({ url, body }) => {
  return post(url, { body }) as Promise<any>;
};
